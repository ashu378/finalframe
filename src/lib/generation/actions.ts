...
export async function createShotGenerationJob(input: { productionId: string; shotId: string; qualityTier?: QualityTier }) {
    ...
    try {
        ...
        const { production, shot } = await convex.query(api.generation.getShot, { productionId: input.productionId as any, shotId: input.shotId as any });
        ...
        const job = await convex.mutation(api.generation.createJob, { productionId: input.productionId as any, shotId: input.shotId as any, provider: capability.provider, model: capability.model, request: { prompt: shot.prompt, duration: shot.durationSeconds, ratio: production.outputPreset }, estimatedCost: estimate.totalCredits, idempotencyKey });
        ...
        if (!reservation.success) {
            await convex.mutation(api.generation.failJob, { jobId: job?._id as any, errorMessage: reservation.error || 'Insufficient credits' });
...
export async function processShotGenerationJob(jobId: string) {
    ...
    try {
        ...
        const completed = await convex.mutation(api.generation.completeJob, { jobId: jobId as any, assetUrl: result.videoUrl, providerJobId: result.taskId, response: result });
        ...
        try { await convex.mutation(api.generation.failJob, { jobId: jobId as any, errorMessage: message }); } catch { /* preserve original provider error */ }
        const job = await convex.query(api.generation.getJob, { jobId: jobId as any }).catch(() => null);
