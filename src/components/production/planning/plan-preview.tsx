...
export function PlanPreview({ preview, balance, outputPreset, qualityTier, isSubmitting, onRevise, onApprove, onBack }: PlanPreviewProps) {
    ...
    return (
        <div className="space-y-6 pb-8">
            <section className="overflow-hidden rounded-[1.5rem] bg-[#0f141c] p-6 sm:p-9" aria-labelledby="plan-preview-title">
            ...
            <nav className="overflow-x-auto rounded-2xl border border-border/70 bg-card p-2" aria-label="Plan sections">
                <div className="flex min-w-max gap-1 text-sm font-semibold">
                    {[['script', 'Script'], ['guide', 'Creative guide'], ['parts', 'Parts & takes'], ['estimate', 'Estimate']].map(([id, label]) => <a key={id} href={`#${id}`} className="rounded-xl px-3 py-2.5 text-muted-foreground transition hover:bg-secondary hover:text-foreground">{label}</a>)}
                </div>
            </nav>

            <section id="script" className="ff-card scroll-mt-5 p-6 sm:p-8">
                <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary text-primary-foreground"><FileText className="size-5" aria-hidden="true" /></span><div><p className="ff-eyebrow">Story and script</p><h3 className="ff-display mt-2 text-2xl font-semibold">This is what the video is saying</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{preview.script.label}. You can ask for a change before approving the plan.</p></div></div>
            ...
            <section id="guide" className="ff-card scroll-mt-5 p-6 sm:p-8">
                <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[hsl(var(--success)/.2)] text-[hsl(var(--success))]"><Lightbulb className="size-5" aria-hidden="true" /></span><div><p className="ff-eyebrow">Creative guide</p><h3 className="ff-display mt-2 text-2xl font-semibold">The details that keep it feeling like one video</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">FinalFrame will use these choices as the reference while it plans each take.</p></div></div>
