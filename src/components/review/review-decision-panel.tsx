  ...
  return (
    <div className="space-y-4" aria-live="polite">
      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
        <span className={`inline-flex items-center gap-2 rounded-full px-3 py-2 ${isApproved ? 'bg-[#9be3c3]/70 text-[hsl(var(--success))]' : isChangesRequested ? 'bg-[#151b25] text-[#f6c85f]' : 'bg-secondary text-muted-foreground'}`}>
          <ShieldCheck className="size-4" aria-hidden="true" />
          {isApproved ? 'Approved' : isChangesRequested ? 'Changes requested' : 'Waiting for your decision'}
        </span>
      </div>
      {isChangesRequested ? <p className="rounded-xl bg-[#151b25] p-4 text-sm leading-6 text-muted-foreground">Your note is saved. FinalFrame will use it to prepare the next version.</p> : null}
