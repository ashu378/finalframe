...
export const recordWebhook = mutation({
  args: { provider: v.string(), providerEventId: v.string(), eventType: v.string(), payload: v.any(), reference: v.optional(v.string()), providerChargeId: v.optional(v.string()) },
  handler: async (ctx, args) => {
    ...
    if (args.eventType !== "collection.succeeded" || !args.reference) return { duplicate: false, eventId };
    const purchase = await ctx.db.query("paymentPurchases").withIndex("by_reference", (q) => q.eq("reference", args.reference!)).unique();
